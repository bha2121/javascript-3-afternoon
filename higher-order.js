...
//Code Here
let totalPopulation = populations.reduce((total, curElement) => {           //  = populations.reduce(/* Provide Your Callback Here */)
  return (total + curElement);
});  
...
let myStrongest = monstersInYourPocket.filter((callback, element) => {     // = monstersInYourPocket.filter(/* Provide Your Callback Here */)
  return monstersInYourPocket[element].CP > 200
});
...
let orderTotals = orders.map((callback, element) => {
  return (orders[element].price * orders[element].tax +orders[element].price);
});                                               
...
*/
//code here
let bobspurchases = purchases.filter( (obj) => obj.owner === 'Bob')

let bobsTotal = bobspurchases.reduce( (total, element) => total + element.price, 0);



// let newbobsTotal = purchases.filter(obj => {
//   purchases[element].owner === "Bob"
// })

// let bobsTotal = newbobsTotal.reduce((total, element) => {
//   return total + newbobsTotal[element].price;
// })
